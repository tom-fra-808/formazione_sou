...
if (( $# > 3 || $# < 3 )); then
	echo "Il programma $0 necessita 3 argomenti: ind.ip porta d'inizio e porta di fine scan."
	echo "ex. '192.168.10.10 200 23000'"
	exit 1
fi
...
echo "====PORTE CHIUSE====" > /tmp/closed.txt
echo "====PORTE APERTE====" > /tmp/open.txt


for (( i=ini_port; i<=fini_port; i++ )); do
	if nc -z -w 1 "$ip" "$i" 2>/dev/null; then
		echo "Porta $i aperta"
		echo "Porta $i" >> /tmp/open.txt
	else
		echo "Porta $i chiusa o non raggiungibile"
		echo "Porta $i" >> /tmp/closed.txt
	fi
...
echo 
	while true; do
		...
		read -r -p "Destinazione: " dest
			...
			cat /tmp/open.txt > "$dest/$nf"
			cat /tmp/closed.txt >> "$dest/$nf"
